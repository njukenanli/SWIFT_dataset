...
class SimpleBatchInstance(BaseModel):
    ...
    @classmethod
    def from_swe_bench(cls, instance: dict[str, Any], islive = False, ispro = False) -> Self:
        ...
        extra_fields["instance"] = instance
        return cls(
...
class SWEBenchInstances(BaseModel, AbstractInstanceSource):
    ...
    def get_instance_configs(self) -> list[BatchInstance]:
        ...
        if os.path.exists(self._get_dataset_path()):
            with open(self._get_dataset_path()) as f:
                ds: list[dict[str, Any]] = [json.loads(i) for i in f]
        else:
            ds: list[dict[str, Any]] = load_dataset("json", data_files=self._get_dataset_path(), split="train")  # type: ignore
        ...
        if "live" in dataset_path:
            instances = [
                SimpleBatchInstance.from_swe_bench(instance, islive = True).to_full_batch_instance(self.deployment) for instance in ds
            ]
        elif "pro" in dataset_path:
            instances = [
                SimpleBatchInstance.from_swe_bench(instance, ispro = True).to_full_batch_instance(self.deployment) for instance in ds
            ]
        else:
            instances = [
