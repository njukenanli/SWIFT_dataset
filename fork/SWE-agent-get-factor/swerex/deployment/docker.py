...
class DockerDeployment(AbstractDeployment):
    ...
    def _get_swerex_start_cmd(self, token: str, call_sh: bool = True) -> list[str]:
    ...
    def glibc_dockerfile(self, ensure_GLIBC_compatible = False) -> str:
        # will only work with glibc-based systems
        if self._config.platform:
            platform_arg = f"--platform={self._config.platform}"
        else:
            platform_arg = ""
        if ensure_GLIBC_compatible:
            builder_base = "$BASE_IMAGE" # from scratch, slow, compatible
        else:
            builder_base = "python:3.11.9-slim-bookworm"  # cached , fast
        return (
            "ARG BASE_IMAGE\n\n"
            # Build stage for standalone Python
            f"FROM {platform_arg} {builder_base} AS builder\n"
            ...
            f"RUN rm -f /root/.config/pip/pip.conf\n\n"
    ...
    def _build_image(self) -> str:
        ...
        try:
            dockerfile = self.glibc_dockerfile(False)
            image_id = (
            ...
            # ensure max compatibility
            print("Falling back to max compatibility build, though slower...")
            dockerfile = self.glibc_dockerfile(True)
            image_id = (
                subprocess.check_output(
                    build_cmd,
                    input=dockerfile.encode(),
        ...
        try:
            cmds = [
                ...
                *self._config.docker_args,
                ...
                image_id,
                *self._get_swerex_start_cmd(token, "jefzda" not in self._container_name),
            ]
            ...
            print("Falling back to /bin/sh start mode for swebench pro corner cases.")
            cmds = [
                self._config.container_runtime,
                "run",
                *rm_arg,
                ...
                *self._config.docker_args,
                ...
                self._container_name,
                image_id,
                *self._get_swerex_start_cmd(token, True),
            ]
            cmd_str = shlex.join(cmds)
            self.logger.info(
                f"Starting container {self._container_name} with image {self._config.image} serving on port {self._config.port}"
