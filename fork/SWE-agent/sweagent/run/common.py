    ...
    def get_config(self, args: list[str] | None = None) -> BaseSettings:
        ...
        if cli_args.config:
            ...
            for _f in cli_args.config:
                ...
                ablation_configs = {}
                if "ablation" in _loaded["agent"].keys():
                    for item in _loaded["agent"]["ablation"]:
                        for k,v in item.items():
                            ablation_configs[k] = v
                    del _loaded["agent"]["ablation"]
        ...
        config._ablation = ablation_configs  # type: ignore
        return config
