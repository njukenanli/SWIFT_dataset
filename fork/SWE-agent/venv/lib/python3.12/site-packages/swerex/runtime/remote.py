...
class RemoteRuntime(AbstractRuntime):
    ...
    async def _request(self, endpoint: str, payload: BaseModel | None, output_class: Any, num_retries: int = 0, timeout: float = 60.0):
    ...
    async def run_in_session(self, action: Action) -> Observation:
        ...
        # Extract timeout from action if it's a BashAction, add buffer for network overhead
        timeout = 1810.0
        if hasattr(action, 'timeout') and action.timeout:
            timeout = action.timeout + 10  # Add 10 second buffer for network/processing overhead
        return await self._request("run_in_session", action, Observation, timeout=timeout)
