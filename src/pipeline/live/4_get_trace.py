    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(future_to_instance):
            ...
            try:
                ...
                with write_lock:
                    with open("data/live/4_test_loc_context.jsonl", "a") as f:
                        f.write(json.dumps(result)+"\n")
            except TimeoutError:
                print(f"Timeout after 30 minutes: {instance['instance_id']}")
                instance["error_context"] = []
                with open("data/pro/pyt/4_test_loc_context.jsonl", "a") as f:
                    f.write(json.dumps(result)+"\n")
...
if __name__ == "__main__":
    ...
    if os.path.exists("data/live/4_test_loc_context.jsonl"):
        with open("data/live/4_test_loc_context.jsonl") as f:
            done = [json.loads(i)["instance_id"] for i in f]
    with open("data/live/3_verified_test_loc.jsonl") as f:
