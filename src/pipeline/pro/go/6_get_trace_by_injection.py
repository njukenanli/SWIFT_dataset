...
from src.utils.go.cov import StackTraceExtractorByInjection, Stack
...
def proc_instance(instance):
    ...
    log = logger(f"data/exec_logs/{instance["instance_id"]}.txt")
    log(f">>>>>>>>>>>>>>>>{instance["instance_id"]}\n")
    log(instance["instance_id"] + " deleted lines:" + json.dumps(tgt_locs, indent=True) + "\n")
    ...
    if tgt_locs:
        log(instance["instance_id"] + " Case 1\n")
        ...
        log("hit2")
        log(f'{instance["instance_id"]} {[len(i) for i in trace_list]}\n')
        container.cleanup()
    if (not tgt_locs) or (not trace_list):
        log(instance["instance_id"] + " Case 2\n")
        container: SetupRuntime = SetupRuntime.from_launch_image(image, instance["instance_id"], "linux", None, "/app")
        added_locs = get_added_loc(instance["patch"]) # post solution patch
        log(instance["instance_id"] + json.dumps(added_locs, indent=True) + "\n")
        neighbor_locs = get_neighbor_loc(instance["patch"]) # pre patch
        log(instance["instance_id"] + json.dumps(neighbor_locs, indent=True) + "\n")
        patch =  instance["patch"]  # + instance["test_patch"] + "\n\n"
        # when test_patch and patch are both applied, all tests should pass!!!
        ...
        log(f'{instance["instance_id"]} {[len(i) for i in added_trace_list]}\n')
        ...
        log(f'{instance["instance_id"]} {[len(i) for i in neighbor_trace_list]}\n')
        trace_list = pre_tracer.merge_trace_by_func(trace_list+neighbor_trace_list)
        log(f'{instance["instance_id"]} {[len(i) for i in trace_list]}\n')
        container.cleanup()
    log("hit5")
    assert len(trace_list) > 0, "No valid trace found..."
    log(f'{instance["instance_id"]} {json.dumps(trace_list, indent=True)}\n')
    log(f"<<<<<<<<<<<<<<<<<{instance["instance_id"]}\n\n\n")
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for future in as_completed(future_to_instance):
            ...
            try:
                ...
                with write_lock:
                    with open("data/pro/go/5_test_loc_context_api.jsonl", "a") as f:
