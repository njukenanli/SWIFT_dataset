...
def get_api_loc(cov: list[CovInfo], 
    ...
    for interval, order_in_cov in cov_idx:
        if interval[0]<=appear_path_lineno<=interval[1]:
            for cov_info in cov[order_in_cov:]:
...
def main(instances):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in as_completed(future_to_instance):
            ...
            try:
                result, error = future.result(timeout=1200)
                if error:
                    print(error, flush=True)
                elif result:
                    with open("data/verified/6_test_loc_context_api.jsonl", "a") as f:
...
if __name__ == "__main__":
    ...
    if os.path.exists("data/verified/6_test_loc_context_api.jsonl"):
